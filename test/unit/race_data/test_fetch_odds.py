...
def test_fetch_odds_raises_on_race_day_without_odds() -> None:
    """レース当日にオッズが無ければ予想オッズに切り替えず DataNotFoundError."""
    today_race_code = datetime.date.today().strftime("%Y%m%d") + "05050801"
    mock_di = make_mock_di()
...
def test_fetch_odds_propagates_expected_odds_error() -> None:
