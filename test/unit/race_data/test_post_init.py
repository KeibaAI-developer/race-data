...
from .conftest import (
    ...
    _PAST_RACE_CODE,
    _make_mock_di,
    _make_past_performances_df,
    _make_race_basic_info_df,
    _make_win_show_odds_df,
)
...
def test_race_basic_info_df_fetched(past_race_data: RaceData, mock_di_past: MagicMock) -> None:
    ...
    mock_di_past.get_race_basic_info.assert_called_once_with(_PAST_RACE_CODE)
...
def test_entry_df_fetched_for_past_race(past_race_data: RaceData, mock_di_past: MagicMock) -> None:
    ...
    mock_di_past.get_entry.assert_called_once_with(_PAST_RACE_CODE)
...
def test_entry_df_fetched_for_future_race(
    ...
    mock_di_future.get_entry.assert_called_once_with(_FUTURE_RACE_CODE)
...
def test_win_show_odds_df_fetched(past_race_data: RaceData, mock_di_past: MagicMock) -> None:
    ...
    mock_di_past.get_win_show_odds.assert_called_once_with(_PAST_RACE_CODE)
...
def test_result_df_fetched_for_past_race(
    ...
    mock_di_past.get_result.assert_called_once_with(_PAST_RACE_CODE)
...
def test_race_result_info_df_fetched_for_past(
    ...
    mock_di_past.get_race_result_info.assert_called_once_with(_PAST_RACE_CODE)
...
def test_payoff_df_fetched_for_past(past_race_data: RaceData, mock_di_past: MagicMock) -> None:
    ...
    mock_di_past.get_payoff.assert_called_once_with(_PAST_RACE_CODE)
...
def test_num_runners_fallback_to_win_show_odds() -> None:
    ...
    mock_di = _make_mock_di(
        race_basic_info_df=_make_race_basic_info_df(shutsu_count=None),
        win_show_odds_df=_make_win_show_odds_df(ninkis=[1, 2, None]),
    )
    race_data = RaceData(race_code=_PAST_RACE_CODE, data_interface=mock_di)
...
def test_baba_code_auto_set_from_dirt() -> None:
    ...
    mock_di = _make_mock_di(
        race_basic_info_df=_make_race_basic_info_df(
            shiba_da="ダ",
            dart_baba_code="2",
        ),
    )
...
def test_baba_code_overridden_by_constructor() -> None:
    ...
    mock_di = _make_mock_di()
    race_data = RaceData(race_code=_PAST_RACE_CODE, data_interface=mock_di, baba_code="3")
